	...
	public static Map<String, Object> validToken(String token) {
		...
		try {
			...
			if (jwsObject.verify(verifier)) {
				...
				//若payload包含uid字段，则校验同一用户token是否一致
				if (jsonOBj.containsKey("uid")) {
					String uid=jsonOBj.get("uid").toString();
					if(TokenUtil.getMUid(uid)!=null){
						String mToken=TokenUtil.getMUid(uid);
						if(token.equals(mToken)){
							...
							if (jsonOBj.containsKey("ext")) {
								...
								// 过期了
								if (curTime > extTime) {
									//再判断是否在刷新期限内(过期多长时间，10分钟内)
									if(curTime - extTime <= 10*60*1000){
										resultMap.put("state", TokenState.REFURBISH.toString());
									}else{
										resultMap.clear();
